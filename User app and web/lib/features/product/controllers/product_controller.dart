...
class ProductController extends GetxController implements GetxService {
  ...
  Future<void> getPopularProductList(bool reload, bool notify, {DataSourceEnum dataSource = DataSourceEnum.local, bool fromRecall = false, bool withFilter = true}) async {
    _filterDataModel ??= FilterDataModel();
    if(reload) {
    ...
    if(_popularProductList == null || reload || fromRecall) {
      ...
      if(dataSource == DataSourceEnum.local) {
        productModel = await productServiceInterface.getPopularProductList( source: DataSourceEnum.local, filterDataModel: withFilter ? _filterDataModel : null);
        _filterDataModel?.minPrice == productModel?.minPrice;
        _filterDataModel?.maxLimit == productModel?.maxPrice;
        _preparePopularProductList(productModel?.products);
        getPopularProductList(false, false, dataSource: DataSourceEnum.client, fromRecall: true, withFilter: withFilter);
      } else {
        productModel = await productServiceInterface.getPopularProductList( source: DataSourceEnum.client, filterDataModel: withFilter ? _filterDataModel : null);
