...
class RestaurantController extends GetxController implements GetxService {
  ...
  Future<void> getRestaurantList(int offset, bool reload, {bool fromMap = false, DataSourceEnum source = DataSourceEnum.local}) async {
    ...
    if(source == DataSourceEnum.local && offset == 1) {
      restaurantModel = await restaurantServiceInterface.getRestaurantList(offset, _restaurantType, _topRated, _discount, _veg, _nonVeg, fromMap: fromMap, source: DataSourceEnum.local, filterDataModel: _filterDataModel);
      _prepareRestaurantList(restaurantModel, offset);
      getRestaurantList(1, false, fromMap: fromMap, source: DataSourceEnum.client);
    } else {
      restaurantModel = await restaurantServiceInterface.getRestaurantList(offset, _restaurantType, _topRated, _discount, _veg, _nonVeg, fromMap: fromMap, source: DataSourceEnum.client, filterDataModel: _filterDataModel);
      _prepareRestaurantList(restaurantModel, offset);
    }
  }
  ...
  Future<void> getPopularRestaurantList(bool reload, bool notify, {DataSourceEnum dataSource = DataSourceEnum.local, bool fromRecall = false, bool withFilter = true, String? query}) async {
    if (reload) {
    ...
    if (_popularRestaurantList == null || reload || fromRecall) {
      ...
      _filterDataModel ??= FilterDataModel();
      _filterDataModel!.name = query;
      if (dataSource == DataSourceEnum.local) {
        popularRestaurantList = await restaurantServiceInterface.getPopularRestaurantList(source: DataSourceEnum.local, filterDataModel: withFilter ? _filterDataModel : null);
        _preparePopularRestaurantList(popularRestaurantList);
        getPopularRestaurantList(false, false, dataSource: DataSourceEnum.client, fromRecall: true, withFilter: withFilter, query: query);
      } else {
        popularRestaurantList = await restaurantServiceInterface.getPopularRestaurantList(source: DataSourceEnum.client, filterDataModel: withFilter ? _filterDataModel : null);
        _preparePopularRestaurantList(popularRestaurantList,);
      }
    }
  }
